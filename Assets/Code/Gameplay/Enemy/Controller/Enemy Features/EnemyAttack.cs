...
{
    public class EnemyAttack : EnemyFeature, IProcess<HitInfo>
    {
        ...
        private EnemyPhysicsCheck physics;
        ...
        private List<string> _targetedTags;
        ...
        [Header("Suck To Target Settings")] [SerializeField]
        ...
        [Header("Aim Distance Calculation")] [SerializeField]
        private float _aimAngleWeight;
        ...
        [Header("Tuning Settings")] [SerializeField]
        ...
        public override void InitializeFeature(Controller controller)
        {
            ...
            if (controller is EnemyController enemy) animator = enemy.Animator;
            MultiplierReset();
            CacheBodyParts();
            _hitboxPool = new(4, this, attributes, _invoker.Forward);
        ...
        private void CheckTargetForSnap(FullAttack attack)
        {
            ...
            var floorPosition = _invoker.Position.Get();
            ...
            foreach (var collission in collissions)
            {
                ...
                var center = _invoker.Center.Get();
        ...
        private void DownAttack()
        {
            ...
            _invoker.Knockback.Execute(Vector3.down * _currentAttack.downImpulse);
        }
        ...
        private void FollowUp(PlayerController closest, FullAttack attack)
        {
            ...
            if (closest == null)
            {
                var targetCenter = _invoker.Center.Get();
                var center = _invoker.Center.Get();
                direction = (targetCenter - center).With(y: 0).normalized;
            }
            else
            {
                direction = _invoker.Forward.Get();
            }

            _invoker.Knockback.Execute(direction * followUpForce.x + Vector3.up * followUpForce.y);
        }
        ...
        public void Dash()
        {
            ...
            var direction = _invoker.Forward.Get();
            _invoker.Knockback.Execute(direction * _currentAttack.dashImpulse);
        }
        ...
        public void Shoot(int tick)
        {
            ...
            if (!_currentAttack.shootAttack || tick >= _currentAttack.attacks.Count) return;

            var forward = _invoker.Forward.Get();
            var shootPosition = _bodyPartsDictionary[_currentAttack.attacks[tick].bodyPartName].position +
                                forward * _shootOffset;
            var projectile = ObjectPoolManager.Instance
                .Get(_currentAttack.projectile, shootPosition, Quaternion.identity).GetComponent<EnemyProjectile>();
        ...
        public void Interruption(bool success)
        {
            ...
            _releaseMultiplier =
                Mathf.Min((Time.time - _waitTime) / lastAttack.chargeTime * lastAttack.chargeMultiplier,
                    lastAttack.chargeMultiplier) + 1;
        }
        ...
        public void WaitForGround()
        {
            ...
            _waiting = false;
            _releaseMultiplier =
                Mathf.Min((Time.time - _waitTime) / _currentAttack.downTime * _currentAttack.downMultiplier,
                    _currentAttack.downMultiplier) + 1;
